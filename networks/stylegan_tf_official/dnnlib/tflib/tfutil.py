...
def is_tf_expression(x: Any) -> bool:
    ...
    return isinstance(x, (tf.compat.v1.Tensor, tf.Variable, tf.Operation))
...
def flatten(x: TfExpressionEx) -> TfExpression:
    ...
    with tf.compat.v1.name_scope("Flatten"):
        return tf.compat.v1.reshape(x, [-1])


def log2(x: TfExpressionEx) -> TfExpression:
    """Logarithm in base 2."""
    with tf.compat.v1.name_scope("Log2"):
        return tf.compat.v1.log(x) * np.float32(1.0 / np.log(2.0))


def exp2(x: TfExpressionEx) -> TfExpression:
    """Exponent in base 2."""
    with tf.compat.v1.name_scope("Exp2"):
        return tf.compat.v1.exp(x * np.float32(np.log(2.0)))
...
def init_uninitialized_vars(target_vars: List[tf.compat.v1.Variable] = None) -> None:
        ...
        try:
            ...
            with absolute_name_scope(var.name.split(":")[0]):
                with tf.control_dependencies(None):  # ignore surrounding control_dependencies
                    setter = tf.assign(var, tf.compat.v1.placeholder(var.dtype, var.shape, "new_value"), name="setter")  # create new setter
