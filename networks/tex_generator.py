...
class G_tex(nn.Module):
	def __init__(self, vgg_model):
		...
		self.encoder2 = nn.Sequential(
			...
			nn.Conv2d(256, 512*5, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), padding_mode='reflect'),
			nn.ReLU(inplace=True)
			)
		...
		self.decoder = nn.Sequential(
			nn.Conv2d(512*5, 2048, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), padding_mode='reflect'),
			nn.ReLU(inplace=True),
	...
	def forward(self,  x, y, alpha=1.0):
		...
		encode_style = torch.cat((self.encodera(y[0]),self.encoderb(y[1]),self.encoderc(y[2]),self.encoderd(y[3]), self.encodere(y[4])), 1)
